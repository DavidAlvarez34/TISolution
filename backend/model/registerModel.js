...
module.exports = class loginModel {
  ...
  async create(theUser) {
    try {
      console.log("El nuevo usuario", theUser);
      let result = await sequelize.query(
        "INSERT INTO userTiBudget (emailUser,userPassword ) VALUES ('" +
          theUser.sendEmail +
          "','" +
          theUser.sendPassword +
          "');"
      );
      console.log(result);
      return result;
  ...
  async find(loginId) {
    let result = await sequelize.query(
      "SELECT * FROM usuario WHERE idUsuario = " + loginId
    );
    return result[0][0];
  }
  ...
  async update(updateAPassword) {
    ...
    let result = await sequelize.query(
      "UPDATE userTiBudget SET userPassword = '" +
        updateAPassword.paswordChange +
        "'" +
        " WHERE emailUser = '" +
        updateAPassword.email +
        "' AND userPassword = '" +
        updateAPassword.paswordCurrent +
        "'"
    );
    console.log(result);
    return result;
